...
fn check_versions() {
    ...
    let chart_version = extract_version(&chart, "version:").unwrap_or_else(|| {
        eprintln!("No version found in {HELM_CHART_FILE_PATH}");
        process::exit(1)
    });
    let app_version = extract_version(&chart, "appVersion:").unwrap_or_else(|| {
        eprintln!("No appVersion found in {HELM_CHART_FILE_PATH}");
        process::exit(1)
    });
...
fn release(version: &str) {
    ...
    run(
        ...
        &[
            "add",
            CARGO_FILE_PATH,
            CARGO_LOCK_FILE_PATH,
            HELM_CHART_FILE_PATH,
            CHANGELOG_FILE_PATH,
        ],
    );
    run(
